...
class MotorController:
    def __init__(self, robot, physicsClientId, timeStep, kp, kd, torque,
                 max_velocity):
        self._robot = robot
        ...
        for i in range(
                p.getNumJoints(self._robot,
                               physicsClientId=self._physicsClientId)):
            jointInfo = p.getJointInfo(self._robot,
                                       i,
                                       physicsClientId=self._physicsClientId)
            if jointInfo[2] == 0:
                joint_id_list.append(jointInfo[0])
                joint_pos_list.append(
                    p.getJointState(self._robot,
                                    jointInfo[0],
                                    physicsClientId=self._physicsClientId)[0])
    ...
    def setMotorsAngleInFixedTimestep(self, motorTargetAngles, motorTargetTime,
                                      delayTime):
        if (motorTargetTime == 0):
            ...
            if (delayTime < 0):
                ...
                self.checkCurrentPose()
                self._joint_targetPos = np.array(motorTargetAngles)
                for i in range(self._joint_number):
                    dydt = (self._joint_targetPos -
                            self._joint_currentPos) / motorTargetTime
                internalTime = 0.0
                while internalTime < motorTargetTime + delayTime:
    ...
    def setMotorsAngleInFixedTimestepSyncRealTime(self,
                                                  motorTargetAngles,
                                                  motorTargetTime,
                                                  delayTime,
                                                  speed=1):
                ...
                while internalTime < motorTargetTime + delayTime:
                    ...
                    for i in range(self._joint_number):
                        p.setJointMotorControl2(
                            ...
                            physicsClientId=self._physicsClientId)
                    p.stepSimulation(physicsClientId=self._physicsClientId)
                    while ((time.time() - reftime) * speed < internalTime):
                        pass
            else:
                #self._joint_currentPos = self._joint_targetPos
                self.checkCurrentPose()
    ...
    def NoCommandSyncRealTime(self, waitTime, speed=1):
        internalTime = 0.0
        reftime = time.time()
        while internalTime < waitTime:
            p.stepSimulation(physicsClientId=self._physicsClientId)
            internalTime += self._timeStep
            while ((time.time() - reftime) * speed < internalTime):
                pass

    def NoCommand(self, waitTime):
        for _ in range(int(waitTime / self._timeStep)):
            p.stepSimulation(physicsClientId=self._physicsClientId)
