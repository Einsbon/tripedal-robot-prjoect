...
p.resetDebugVisualizerCamera(cameraDistance=0.4,
                             cameraYaw=30,
                             cameraPitch=-15,
                             cameraTargetPosition=[-0.05, 0, 0.1])
...
motorsController.NoCommandSyncRealTime(3, 1)

f = open(os.path.abspath(os.path.dirname(__file__)) + '/testMotion.CSV',
         'r',
         encoding='utf-8')
rdr = csv.reader(f, quoting=csv.QUOTE_NONNUMERIC)
for line in rdr:
    print(list(np.multiply(line[0:9], (math.pi / 180))), line[9] / 1000,
          line[10] / 1000)
    motorsController.setMotorsAngleInFixedTimestepSyncRealTime(
        np.multiply(line[0:9], (math.pi / 180)), line[9] / 1000,
        line[10] / 1000, 1.0)

motorsController.NoCommandSyncRealTime(10, 1)
